...
SERVER_BUILD := pokemon-rumble-world
endif
...
ifeq ($(wildcard .env),)
	...
	go get -u
	go mod tidy
	go build -ldflags "-X 'main.serverBuildString=$(BUILD_STRING)'" -o ./build/pokemon-rumble-world

docker:
	docker build -t pokemon-rumble-world --build-arg BUILD_STRING="$(BUILD_STRING)" .
